...
public class MainFlowExecutionTests extends AbstractXmlFlowExecutionTests {
	...
	protected FlowDefinitionResource getResource(
		...
		return resourceFactory
				.createFileResource("src/main/webapp/WEB-INF/flows/main/main-flow.xml");
	}
	...
	public void testStartMainFlow() {
		...
		bookings.add(new Booking(new Hotel(), new User("keith", "password",
				"Keith Donald")));
		EasyMock.expect(bookingService.findBookings("keith")).andReturn(
				bookings);
		...
		mockBookingFlow.setInputMapper(new Mapper() {
			public MappingResults map(Object source, Object target) {
				assertEquals(new Long(1),
						((AttributeMap) source).get("hotelId"));
				return null;
			}
		});
